...
void drawWalls(){
	//cout << "drawWalls - isSelecting : " << isSelecting << endl;
...
void drawDoor(){
	//cout << "drawDoor - isSelecting : " << isSelecting << endl;
...
void drawSpheres(){
	//cout << "drawSpheres - isSelecting : " << isSelecting << endl;
	//cout << "closestName: " << closestName << endl;
...
void pickFunction(int button, int state, int x, int y)
{
	...
	gluPickMatrix((float)x, (float)(viewport[3] - y), 3.0, 3.0, viewport);
	cout << "camera coord: (" << camX << ", " << camY << ", " << camZ << ")" << endl;
	gluPerspective(45.0f, (double)scrW / scrH, 1.0, 1000.0);

	glMatrixMode(GL_MODELVIEW); // Return to modelview mode before drawing.m
	glLoadIdentity();
